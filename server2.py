...
# Listen to accept incoming connections which are not already in connection_list
def listen():
    ...
    while True:
        try:
            ...
            if conn not in connection_list:
                ...
                receive_messages_thread = Thread(target=handle_client, args=(conn, addr), daemon=True)
 ...
 #   s.listen(5)
  #  print("socket is listening...")
   ...
   # if not shutdown:
        ...
        #start 3 threads as
    listen_to_connections_thread = Thread(target=listen, daemon=True)
  ...
  #  send_messages = Thread(target=send, daemon=True)
    ...
    listen_to_connections_thread.start()
  ...
  #  send_messages.start()
    ...
    listen_to_connections_thread.join()
